...
public interface RevisitDao {
	...
	public int delete(int id);
	 ...
	 @Insert("insert into Revisit (time,customerid,linkstatus,customerstatus,purposestatus,assessstatus,askinfo,followinfo,status,comments) values(#{time},#{customerid},#{linkstatus},#{customerstatus},#{purposestatus},#{assessstatus},#{askinfo},#{followinfo},#{status},#{comments})")
	 public Integer insert(Revisit t);

	 @Update("update Revisit set time=#{time},customerid=#{customerid},linkstatus=#{linkstatus},customerstatus=#{customerstatus},purposestatus=#{purposestatus},assessstatus=#{assessstatus},askinfo=#{askinfo},followinfo=#{followinfo},status=#{status},comments=#{comments} where id=#{id}")
